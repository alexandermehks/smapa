...
def serviceB():
	try:
		...
		if request.method == 'POST':
			...
			database.insertStartDate(customer_id,start,service="b")

			c =  customer.Customer(customer_id,start)
			ServiceB = B(customer_id)

			discount = ServiceB.discount
			...
			if customer_db["price-B"] != 0:
				ServiceB.base_price = customer_db["price-B"]
