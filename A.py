...
class A():
	def __init__(self,customer_id, base_price=0.2, discount = 0):
...
def serviceA():
	try:
		...
		if request.method == 'POST':
			...
			database.insertStartDate(customer_id,start,service="a")

			c =  customer.Customer(customer_id,start)
			ServiceA = A(customer_id)

			discount = ServiceA.discount
			...
			if customer_db["price-A"] != 0:
				ServiceA.base_price = customer_db["price-A"]
			...
			if discount != 0:
				new_discounted_price = (100-discount)/100*ServiceA.base_price
				ServiceA.base_price = new_discounted_price
			...
			total_price =  days * ServiceA.base_price
